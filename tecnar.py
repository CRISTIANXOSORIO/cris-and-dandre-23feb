...
def Getdata():
    ...
    if Ln3 == 1:
        ...
        x =tk.Label(frame, text="indefinido")
        x.pack()
...
ventana.title("registro tecnar")
ventana.configure(background="white")
ventana.geometry("1280x720")

lname = tk.Label(ventana, text="Nombre: ")
...
llast = tk.Label(ventana, text="Apellido: ")
...
lage = tk.Label(ventana, text="Edad: ")

lage.grid(column=0, row=3,sticky="w", pady= 5)

cage = tk.Entry(ventana,bg="white")
...
rna = tk.Radiobutton(ventana, text="indefinido", value=3, state="normal", variable=sexv)

rna.grid(column=3,row=5,pady=5)

city = tk.Listbox(ventana, width=30, selectmode="single")

city.insert(1, "cartagena")

city.insert(2, "monteria")

city.insert(3, "barranquilla")

city.insert(4, "bogota")

city.grid(column=0, row=6, pady=5)

rbutton= tk.Button(ventana, text="Registro", command=Getdata)
